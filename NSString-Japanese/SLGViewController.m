...
- (void)viewDidLoad
{
    ...
    NSString *jaText = @"高野たかの たかの たかの たかの";
    //@"『もののけ姫』（もののけひめ）は、宮崎駿によるスタジオジブリの長編アニメーション映画作品。1997年（平成9年）7月12日公開。森を侵す人間たちとあらぶる神々との対立を背景として、狼に育てられた「もののけ姫」と呼ばれる少女サンとアシタカとの出会いを描く。宮崎が構想16年、制作に3年をかけた大作であり、興行収入193億円を記録し当時の日本映画の興行記録を塗り替えた";
    ...
    NSLog(@"%@",[jaText stringByReplacingJapaneseKanjiWithHiragana]);
    NSMutableString *katakanaString  = [NSMutableString stringWithString:[jaText stringByReplacingJapaneseKanjiWithHiragana]];
    CFStringTransform((__bridge CFMutableStringRef)katakanaString, NULL, kCFStringTransformHiraganaKatakana, NO);
    NSLog(@"%@",katakanaString);
